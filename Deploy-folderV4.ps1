...
function Write-CMTraceLog {
    ...
    Add-Content -Path "C:\Temp\Deploy-PMPC.log" -Value $LogLine
}
...
$Log = "C:\scripts\Deploy-folder.log"
...
#     otherwise, if only the 06:00 available slot has passed, make it available now
$Now           = Get-Date
$AvailableTime = (Get-Date -Hour 6 -Minute 0 -Second 0)
$DeadlineTime  = (Get-Date -Hour 9 -Minute 0 -Second 0)
...
if ($Now -ge $DeadlineTime) {
    ...
    # Deadline is still ahead today, but the 06:00 available slot has already
...
foreach ($AppCI in $Apps) {
    ...
    $isRequired  = $Category -contains "Required"
    $isAvailable = $Category -contains "Available"
